...
export default function EventsTabs({ events }: { events: AdminEvent[] }) {
  ...
  const upcoming = events.filter((e: AdminEvent) => new Date(e.date) >= now);
  const archived = events.filter((e: AdminEvent) => new Date(e.date) < now);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {(active === "upcoming" ? upcoming : archived).map((event: AdminEvent) => (
